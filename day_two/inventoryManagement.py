files = '''tjxmoewpqkyaiqvmndgflunszc
...
tjxmoewpqkyaizcrndgfnubszc'''.strip().split('\n')

totalsDict = {'twoCount': 0, 'threeCount': 0}


def maxLetters(boxID):
    found = {
        'twos': False,
        'threes': False
    }
    ...
    for letter in letterSet:
        count = countLetter(letter, boxID)
        if (count == 2 and not found['twos']):
            found['twos'] = True
            totalsDict['twoCount'] += 1
        elif (count == 3 and not found['threes']):
            found['threes'] = True
            totalsDict['threeCount'] += 1
        if (found['twos'] and found['threes']):
            break


def countLetter(letter, fullWord):
    count = 0
    for i in fullWord:
        if i == letter:
            count += 1
...
def removeDifferences(stringsArray):
    basestring = ''
    for i in range(len(stringsArray[0])):
        if stringsArray[0][i] == stringsArray[1][i]:
            basestring += stringsArray[0][i]

    return basestring

def main():
    for contentID in files:
        maxLetters(contentID)
    print(totalsDict['twoCount'] * totalsDict['threeCount'])
    print(removeDifferences(findSimilarId(files)))
